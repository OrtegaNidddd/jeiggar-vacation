...
export default function PlansBenefits({ content }: PlansBenefitsProps) {
  return (
    <section data-aos="fade-up" className="rounded-3xl border border-slate-200 bg-white px-5 py-10 shadow-sm sm:px-8 md:px-12">
      ...
      <div className="mt-10 grid gap-8 md:grid-cols-3">
        {content.items.map(({ title, description, icon: Icon }, index) => (
          <article key={title} data-aos="fade-up" data-aos-delay={index * 70} className="text-center">
