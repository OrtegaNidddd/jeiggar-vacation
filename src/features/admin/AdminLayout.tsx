...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-(--bg-muted) flex">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-30 w-72 bg-white border-r border-border flex flex-col transition-transform duration-200 lg:translate-x-0 ${
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 h-16 border-b border-border shrink-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-(--text) truncate leading-none">Jeiggar Admin</p>
            <p className="text-xs text-(--text-muted) truncate mt-0.5">Gestión de destinos y contenido</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Content */}
        <main className="flex-1 overflow-y-auto bg-(--bg-muted) p-4 lg:p-6">
          <div className="mx-auto w-full max-w-7xl">
            <div className="mb-4 hidden lg:flex items-center justify-between gap-4 rounded-3xl border border-border bg-white p-4 shadow-(--shadow-sm)">
              <div>
                <p className="text-sm text-(--text-muted)">Bienvenido de nuevo al panel</p>
                <h2 className="text-lg font-semibold text-(--text)">Administra tus destinos y el contenido de la página</h2>
              </div>
            </div>
            <Outlet />
          </div>
